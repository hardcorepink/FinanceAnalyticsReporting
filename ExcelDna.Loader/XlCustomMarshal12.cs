...
namespace ExcelDna.Loader
{
    ...
    public unsafe struct XlString12
    {
        [FieldOffset(0)]
    ...
    public unsafe struct XlOper12
    {
        ...
        [StructLayout(LayoutKind.Sequential)]
        unsafe public struct XlBigDataTry
        {
            public byte* hData;    // Handle or byte*, but we only use it as a handle.
            public uint cbData;
        }
        ...
        [FieldOffset(0)]
        public XlBigDataTry bigDataTry;
        [FieldOffset(24)]
        ...
        unsafe public object MarshalNativeToManaged(IntPtr pNativeData)
        {
            ...
            switch (type)
            {
                ...
                case XlType12.XlTypeBigData:
                    //LANDS HERE WHEN HAS A RESULT TYPE OF BIG DATA

                    //get the pointer from bigData strucutre the hData is a pointer to a pointer of the data
                    byte* dref1 = (byte*)pOper->bigDataTry.hData;
                    
                    //deref the pointer
                    var deref2 = (IntPtr)Marshal.PtrToStructure((IntPtr)dref1, typeof(IntPtr));
                    
                    //get the length of the data and assign an array for the length
                    uint length = pOper->bigDataTry.cbData;                                        
                    byte[] managedArray = new byte[length];          
                    
                    //copy the data into the newly defined managed array
                    Marshal.Copy(deref2, managedArray, 0, managedArray.Length);                    
    ...
    public class XlObjectArray12Marshaler : ICustomMarshaler, IDisposable
    {
        ...
        internal class XlObjectArray12MarshalerImpl : IDisposable
        {
            ...
            unsafe public IntPtr MarshalManagedToNative(object ManagedObj)
            {
                ...
                for (int i = 0; i < rows * columns; i++)
                {
                    ...
                    else if (obj is byte[])
                    {
                        ...
                        //LANDS HERE WHEN SETTING UP ARRAY TO SEND DATA TO EXCEL
                        byte[] dataArray = (byte[])obj;
                        //dataArray is pinned
                        //get length of byte array

                        int size = Marshal.SizeOf(dataArray[0].GetType()) * dataArray.Length;
                        bigDataPointer = Marshal.AllocHGlobal(size);

                        //copy the array to unmanaged memory                        

                        Marshal.Copy(dataArray, 0, bigDataPointer, dataArray.Length);

                        byte* ptr = (byte*)bigDataPointer.ToPointer();

                        pOper->bigDataTry.hData = ptr;
                        pOper->bigDataTry.cbData = (uint)size;
                        pOper->xlType = XlType12.XlTypeBigData;
            ...
            public void Reset(bool disposeNested)
            {
                ...
                if(bigDataPointer != IntPtr.Zero)
                {
                    Marshal.FreeHGlobal(bigDataPointer);
                    bigDataPointer = IntPtr.Zero;
                }
            }
